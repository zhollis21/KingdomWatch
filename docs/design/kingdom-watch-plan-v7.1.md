...
Three things this is *not*. It is not movement with a cause: sites are infinite and identical until #26 makes foraging exhaust them, so a band moves because nomads move, toward the best land in reach, and the twenty days is a placeholder cadence. It is not seasonal pressure, which waits on #53. And it is not scouting or fog of war: nothing in this plan has either, `Jobs` already finds the nearest forest without anyone looking, and a council that knows what is within a day's walk is the same assumption. Seasonal pressure joins the trigger when seasons exist; site selection with scoring, and a party that walks to the site, are #35's.
