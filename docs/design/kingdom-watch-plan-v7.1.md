...
One rule is broader than §5 states it. "Every scheduler reference resolves" is asked of both sides: of the queue, that each event still due names a primary entity that resolves — a person, a band or settlement, or a household, since seven of the thirteen scheduled kinds in use are owned by a community or a household rather than by a person (the fourteenth, `SocialDecision`, has no owner yet); and of the systems, that every id a periodic stream recorded as booked is an id the queue still holds. The second half is what #80 exists because of, and it covers all ten places the pattern is used — three fields on `PersonRecord` and seven records the streams keep for the communities they run for (`Warmth`'s evening fires the seventh, since #53).
