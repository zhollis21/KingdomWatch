...
- **An allocation test's warm-up must actually reach the working set the measured pass needs.** Count what the measured pass will produce — events published, entries queued — and check it against every preallocated capacity by arithmetic, not by eye. A capacity that is off by a handful (a booking at midnight dispatching twice in one `AdvanceTo`) reports one large growth allocation that looks like a regression in the code under test and is not
...
- **Reach a handler's guards through the scheduler, never by driving `Handle` by hand.** A test that calls a system's `Handle` directly to *run* it — at an hour, or with an event, the scheduler would never produce — makes that call a supported surface, and every later reviewer will find edge cases in it that only that test can reach. On #79 one such test ("a pass run at 07:00") generated four rounds of findings: the rebook hour, its end-of-time guard, pre-dawn assignment, and finally a duplicated daily stream, whose real fix — the state names the event it booked, and any other throws — made the first three unreachable and deleted them. Direct `Handle` calls are for asserting *refusal* (wrong kind, wrong clock, an event the state does not name), which is how every other fixture uses them
