...
**This checklist is intentionally thin right now.** KingdomWatch has almost no code yet (an M0 throwaway prototype in `Game/`, `Core`/`Core.Tests`/`Harness` not yet scaffolded). Grow this list as real conventions emerge over time. A checklist item earns its place by having actually caught something once; don't pre-invent items for patterns that don't exist yet.
