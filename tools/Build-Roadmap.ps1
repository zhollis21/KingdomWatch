...
# Build the graph
# ---------------------------------------------------------------------------
function Remove-AgentFooter([string]$s) {
    # An agent editing a milestone through the GitHub proxy gets its attribution footer appended
    # to the description; it belongs on comments, not on a milestone's one-line summary.
    $s -replace '(?s)\s*---\s*_Generated by \[Claude Code\]\([^)]*\)_\s*$', ''
}
...
$milestones = @($milestoneNodes | Sort-Object { Get-MilestoneOrder $_.title }, title | ForEach-Object {
    [ordered]@{
        ...
        description = Remove-AgentFooter $_.description; issues = [System.Collections.Generic.List[int]]::new()
    }
})
