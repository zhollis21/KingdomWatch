    ...
    public sealed class WorkTaskTests
    {
        ...
        public void Construction_refuses_what_cannot_be_a_task()
        {
            Assert.Multiple(() =>
            {
                ...
                Assert.That(() => new WorkTask(Worker, new EntityId(EntityKind.Person, 7UL), JobKind.Forager, Start, 1L, 1L, 1L, Home, Forest, Completion), Throws.ArgumentException, "a holder is a community, not a person");
                Assert.That(() => new WorkTask(Worker, new EntityId(EntityKind.Settlement, 7UL), JobKind.Forager, Start, 1L, 1L, 1L, Home, Forest, Completion), Throws.Nothing, "a settlement holds a task as a band does");
