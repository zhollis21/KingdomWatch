...
- Read `README.md` and `docs/design/kingdom-watch-plan-v7.1.md` (or its latest version) before making changes — the design doc explains what this game is and why. `docs/design/kingdom-watch-economy-ladder.md` is its companion (#78): the plan argues about the economy, the ladder enumerates it — the resources and their chains, jobs by tier, buildings and their gates, and the six progression milestones as testable conditions. Read it before touching anything economic, and update it in the same change when a decision moves one of those contents.
...
docs/design/       full design and technical plan, plus the economy ladder
```
...
**The tick loop is allocation-checked and timed** (`Core.Tests/Performance/`). Section 18 commits to zero allocations in the tick loop, and `SchedulerSoakTests` holds the scheduler to exactly that: warm up, then `Allocations.Measure(...)` around a further span of `AdvanceTo` calls, asserting zero bytes. When you add a system that runs under `AdvanceTo`, give it the same test — a `foreach` over an interface, a captured closure, or a `params` call in a per-entity path is invisible in review and costs nothing until it is GC pauses on a phone. The same fixture bounds wall time loosely; the bound exists to catch an order-of-magnitude regression, not to police percentages, so if it ever flakes, loosen it once and say so rather than chase it. The same system must also keep `Core.Tests/Validation/LodEquivalenceTests.cs` green (#14): however a run is chunked into `AdvanceTo` calls, it reaches an identical world, so nothing may do work per call rather than per scheduled event. `dotnet run --project Harness -c Release` runs the two-band world for 200 years and prints its chronicle; `-- --seeds 8` sweeps seeds instead, one line each, and `-- --soak 300` runs `Harness/SchedulerSoak.cs`, which exercises the scheduler alone and prints its throughput (sim-years/s, events/s, peak pending).
