...
- GitHub Issues and Milestones track the backlog (see `/milestones` — M0 through M8, matching the design doc's §19). `docs/roadmap/` is the generated view of it: `README.md` lists what is ready to pick up, `next.md` charts the current milestone and the one after, and `graph.json` is the same data for tools. Read `next.md` before choosing work; do not edit anything under `docs/roadmap/` by hand — a workflow regenerates it from GitHub.
...
**Never renumber or reorder an enum whose values are persisted or ordering-significant** — `EntityKind`, `RandomDomain`, `SimulationPhase`, `ScheduledEventKind`, `ResourceKind`, `DomainEventKind`, `ReasonCode`, `MemoryTier`, `AgeStage`, `Sex`. Append instead. Renumbering silently repoints every existing world's saved references, or reorders every event it ever dispatched, which rewrites its history. `ScheduledEventKind` is the sharpest case, because its numeric value *is* the priority that orders two events sharing an instant, phase and primary entity.
...
# Regenerate docs/roadmap/ from GitHub (read-only; the workflow adds -SyncLabels)
