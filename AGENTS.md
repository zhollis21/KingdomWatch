...
**Never renumber or reorder an enum whose values are persisted or ordering-significant** — `EntityKind`, `RandomDomain`, `SimulationPhase`, `ScheduledEventKind`, `ResourceKind`, `DomainEventKind`, `ReasonCode`, `MemoryTier`. Append instead. Renumbering silently repoints every existing world's saved references, or reorders every event it ever dispatched, which rewrites its history. `ScheduledEventKind` is the sharpest case, because its numeric value *is* the priority that orders two events sharing an instant, phase and primary entity.
