...
namespace KingdomWatch.Core.Needs
{
    ...
    public sealed class Hunger : IScheduledEventHandler
    {
        ...
        /// <summary>
        /// Fills <paramref name="into"/> with every holder with meals scheduled, in the order they were
        ...
        /// <summary>
        /// Fills <paramref name="into"/> with every meal this system has
        ...
        public void CopyBookingsTo(List<PendingBooking> into)
        {
            ...
            for (var i = 0; i < _tracked.Count; i++)
            {
                var booked = _tracked[i].PendingMeal;

                if (!booked.IsNone)
                {
                    into.Add(new PendingBooking(
                        _tracked[i].Group.Id, ScheduledEventKind.MealDue, booked));
                }
            }
        }
