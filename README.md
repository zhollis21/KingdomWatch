...
M0 prototyping. Orthographic 3D with sprite villagers has been selected after desktop and Android trials, and a sustained on-device baseline looks healthy (see [docs/town-prototype.md](docs/town-prototype.md)). M1 is underway in `Core/`: entity identity, keyed RNG, person storage, the simulation clock and event scheduler, the resource ledger, domain events, relationships, households and the death cascade, hunger, and the demographic model (births, ageing and natural death). A render-ceiling ramp test and allocation/GC baseline are deferred to M2's stress test. See [Milestones](https://github.com/zhollis21/KingdomWatch/milestones) for M0–M8.
